...
public class BTreeLeaf {
   ...
   private void redistribute(int slot) {
      int level = contents.getFlag();
      if (level == 0) {
         return;
      }

      int leftBlkNum = (slot > 0) ? contents.getChildNum(slot - 1) : -1;
      int rightBlkNum = (slot < contents.getNumRecs() - 1) ? contents.getChildNum(slot + 1) : -1;

      if (leftBlkNum < 0 || rightBlkNum < 0) {
         return;
      }

      BTreeDir leftSibling = new BTreeDir(tx, new BlockId(filename, leftBlkNum), layout);
      BTreeDir rightSibling = new BTreeDir(tx, new BlockId(filename, rightBlkNum), layout);

      Constant splitkey = contents.getDataVal(slot);
      if (leftSibling.getContents().getNumRecs() > rightSibling.getContents().getNumRecs()) {
         while (contents.getDataVal(slot).equals(splitkey))
            slot++;
         splitkey = contents.getDataVal(slot);
      } else {
         while (contents.getDataVal(slot - 1).equals(splitkey))
            slot--;
      }
